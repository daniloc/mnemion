...
**Phase 2 — attribution. ✅ IMPLEMENTED.** §5 and the cookie actor. `created_by`/`updated_by` are kernel columns on every pattern (added to new patterns in `evolution.ts`, backfilled onto existing tables by an `initSchema` migration, treated as kernel columns in `data.ts`/`schema.ts`). `executeMutate` strips any caller-supplied values and stamps them from `DataContext.actor`; the actor is threaded from the MCP session props (`session.ts` → `hive.mutate`/`batchMutate`) and from the browser session cookie (now carrying the member, backward-compatibly — `router.ts`). Internal/ingress writes default to the `owner` sentinel. Attribution rides along in `query`/`resolve`/prime row reads automatically; an explicit "added by {display_name}" label is left as optional future polish.
