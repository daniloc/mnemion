...
- **Versioning works through the `version` facet.** Each `_plugins` entry carries a semver `version`, and it flows verbatim into both `.claude-plugin/marketplace.json` (per plugin) and that plugin's `plugin.json` — the two files Claude Code's auto-update compares to decide whether to pull. Bump `version` when you change a skill and clients pick up the new `skill_md` on next startup. The bump is **manual** (agent- or human-driven via `mutate`); the worker does not auto-increment — auto-versioning is a deliberately deferred question (see [`project-docs/archive/skill-delivery.md`](project-docs/archive/skill-delivery.md)), since auto-bumping on every typo would push an update to every client.
