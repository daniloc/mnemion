...
The coda sharpened this into a tracked quantity: **conventions are failures lurking in the code; we need contracts.** A convention detector now ratchets the convention→contract surface so the gap can only shrink.
